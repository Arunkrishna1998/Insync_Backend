...
class ForgotPasswordView(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            # reset_url = f"http:///127.0.0.1:8000/{uid}/{token}"
...
# Secret keys used to sign the tokens
SECRET_KEY = 'your_secret_key_here'

def generate_access_token(user_details):
    # Define payload for access token with a shorter expiry time (e.g., 15 minutes)
    # print("generate_access_token = ", "user_details['id'] : ",user_details['id'], "user_details['email'] : ", user_details['email'])
    access_payload = {
        'user_id': user_details['id'],
        'email': user_details['email'],
        'exp': datetime.utcnow() + timedelta(minutes=15),  # Expiry time for the access token
    }
    access_token = jwt.encode(access_payload, SECRET_KEY, algorithm='HS256')
    return access_token

def generate_refresh_token(user_details):
    # Define payload for refresh token with a longer expiry time (e.g., 7 days)
    # print("generate_refresh_token = ", "user_details['id'] : ",user_details['id'], "user_details['email'] : ", user_details['email'])
    refresh_payload = {
        'user_id': user_details['id'],
        'email': user_details['email'],
        'exp': datetime.utcnow() + timedelta(days=7),  # Expiry time for the refresh token
    }
